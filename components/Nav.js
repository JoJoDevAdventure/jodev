  ...
  bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen'>
    ...
    backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full '>
      {navData.map((link, index) => {
        return <Link className={`${link.path === pathname && 'text-accent'} relative flex items-center group hover:text-accent transition-all duration-500`} href={link.path} key={index}>
          {/* tooltip */}
